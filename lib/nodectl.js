...
var PROJECT, action, actions, arg, args, assets, child, cluster, coffee, crypto, defaults, dir, e, escapeRegExp, exec, findsByExtPattern, fs, i, isCode, isDir, isFile, j, jade, logger, markup, method, mkdirp, noseinfo, options, origin, output, packages, path, pid, pidfile, print, reloadAllChilds, scriptbin, shasum, spawn, sqwish, stream, stylus, uglify, up, watch, widfile, workers, xid, xidfile, _fn, _fn1, _fn2, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _n, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6;
...
if (actions.start) {
  ...
  if (cluster.isMaster) {
    ...
    if (options.assets && options.output) {
      _ref6 = findsByExtPattern(options.assets, /\.(js|coffee|css|styl|html|jade)$/);
      _fn2 = function(watch) {
        if (isFile(watch)) {
          try {
            if (options.verbose) {
    ...
    if (options.execmaster) {
      scriptbin = (function() {
        switch (path.extname(options.execmaster)) {
          case '.coffee':
            return './node_modules/coffee-script/bin/coffee';
          default:
            return 'node';
        }
      })();
